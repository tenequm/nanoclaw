 ...
 * Agent SDK's own contract (see code.claude.com/docs/en/agent-sdk/
 * agent-loop.md) is that the loop runs until `ResultMessage` —
...
async function triggerTyping(channelType: string, platformId: string, threadId: string | null): Promise<void> {
  log.info('TYPING_DEBUG triggerTyping', {
    channelType,
    platformId,
    threadId,
    hasAdapter: !!adapter,
    hasSetTyping: !!adapter?.setTyping,
  });
  try {
    await adapter?.setTyping?.(channelType, platformId, threadId);
    log.info('TYPING_DEBUG triggerTyping returned', { channelType, platformId });
  } catch (err) {
    log.warn('TYPING_DEBUG triggerTyping threw', { channelType, platformId, err });
  }
}
...
export function startTypingRefresh(
  ...
  const interval = setInterval(() => {
    ...
    if (age > TYPING_TTL_MS) {
      log.warn('TYPING_DEBUG hit TTL ceiling, stopping', { sessionId, ageMs: age });
