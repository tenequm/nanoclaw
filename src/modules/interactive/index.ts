...
async function handleInteractiveResponse(payload: ResponsePayload): Promise<boolean> {
  ...
  // fallback handler (src/commands/fallback.ts), which routes the tap without
  // ever waking a container. Skip them here so registration order between the
  // two handlers never matters and we never route a spurious question_response
  // into a container that never asked.
