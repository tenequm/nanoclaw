 ...
 */
export async function runHostCommand(ctx: HostCommandContext): Promise<void> {
  const targets = await resolveTargets(ctx.mg.id);
  switch (ctx.command) {
    case 'status':
      await handleStatus(ctx, targets);
      return;
    case 'model':
      await handleModel(ctx, targets);
      return;
    case 'config':
      await handleConfig(ctx, targets);
      return;
    case 'restart':
 ...
 */
async function sessionForAgent(ctx: HostCommandContext, agentGroupId: string): Promise<Session | null> {
  ...
  const { session } = await resolveSession(agentGroupId, ctx.mg.id, ctx.event.threadId, mode);
  return session;
}
...
async function writeTextReply(agentGroupId: string, sessionId: string, addr: ReplyAddr, text: string): Promise<void> {
  await writeOutboundDirect(agentGroupId, sessionId, {
    id: `hcmd-reply-${rand()}`,
    kind: 'chat',
...
async function replyText(ctx: HostCommandContext, session: Session, text: string): Promise<void> {
  await writeTextReply(session.agent_group_id, session.id, deliveryAddr(ctx), text);
}

/** Resolve a session for the first target and reply, or log if none exists. */
async function replyOnFirstTarget(ctx: HostCommandContext, targets: TargetResolution, text: string): Promise<void> {
...
async function isAdmin(userId: string | null, agentGroupId: string): Promise<boolean> {
  return userId != null && (await hasAdminPrivilege(userId, agentGroupId));
}
 ...
 */
async function replyMultipleAgents(
...
async function handleStatus(ctx: HostCommandContext, targets: TargetResolution): Promise<void> {
  ...
  const decided = await Promise.all(
    agents.map(async (a) => [a, await statusAccess(ctx.userId, a.agentGroupId)] as const),
  );
  const accessible = decided.filter(([, d]) => d === 'allowed').map(([a]) => a);
  if (accessible.length === 0) {
    if (decided.some(([, d]) => d === 'refuse')) {
      await replyOnFirstTarget(ctx, targets, 'Permission denied: /status requires access to this agent.');
    } else {
  ...
  const blocks = await Promise.all(
    accessible.map(async (a) => {
      const res = await getStatus(a.agentGroupId, chatCtx);
      return res.ok ? renderStatus(res.view) : failureMessage(res);
    }),
  );
  await replyOnFirstTarget(ctx, targets, blocks.join('\n\n'));
}
...
async function handleModel(ctx: HostCommandContext, targets: TargetResolution): Promise<void> {
  if (targets.kind === 'none') {
    logNoAgent('model', ctx);
    return;
  }
  if (targets.kind === 'multiple') {
    await replyMultipleAgents(ctx, targets, 'model');
    return;
  }
  ...
  // gate reads).
  if (ctx.args === '') {
    if (!(await isAdmin(ctx.userId, agent.agentGroupId))) {
      await replyOnFirstTarget(ctx, targets, `Permission denied: /model requires admin access for ${agent.agentName}.`);
      return;
    }
    const res = await getModelPicker(agent.agentGroupId);
    if (!res.ok) {
      await replyOnFirstTarget(ctx, targets, failureMessage(res));
      return;
    }
    await emitModelPicker(ctx, res.view);
    return;
  }
 ...
 */
async function emitModelPicker(ctx: HostCommandContext, view: ModelPickerView): Promise<void> {
  const session = await sessionForAgent(ctx, view.agentGroupId);
  if (!session) {
...
async function handleConfig(ctx: HostCommandContext, targets: TargetResolution): Promise<void> {
  if (targets.kind === 'none') {
    logNoAgent('config', ctx);
    return;
  }
  if (targets.kind === 'multiple') {
    await replyMultipleAgents(ctx, targets, 'config');
    return;
  }
  ...
  if (ctx.args === '') {
    if (!(await isAdmin(ctx.userId, agent.agentGroupId))) {
      await replyOnFirstTarget(
        ctx,
        targets,
        `Permission denied: /config requires admin access for ${agent.agentName}.`,
      );
      return;
    }
    const res = await getConfigView(agent.agentGroupId, chatContext(ctx));
    await replyOnFirstTarget(ctx, targets, res.ok ? renderConfigView(res.view) : failureMessage(res));
    return;
  }
  ...
  if (parsed.field === 'activation' || parsed.field === 'pattern') {
    ...
    if (parsed.field === 'pattern') {
      res = await setActivation(ctx.mg.id, agent.agentGroupId, 'pattern', parsed.value, ctx.userId ?? '');
      ...
      res = await setActivation(ctx.mg.id, agent.agentGroupId, parsed.value, null, ctx.userId ?? '');
    }
    await replyOnFirstTarget(
      ctx,
      targets,
...
async function handleRestart(ctx: HostCommandContext, targets: TargetResolution): Promise<void> {
  if (targets.kind === 'none') {
    logNoAgent('restart', ctx);
    return;
  }
  if (targets.kind === 'multiple') {
    await replyMultipleAgents(ctx, targets, 'restart');
    return;
  }
...
async function handleHostCommandResponse(payload: ResponsePayload): Promise<boolean> {
  ...
  if (!(await hasTable(getDb(), 'pending_questions'))) return true; // ours, but nowhere to land
  ...
  if (!actor || !(await hasAdminPrivilege(actor, agentGroupId))) {
    log.warn('Non-admin tapped a host-command card', { questionId: payload.questionId, userId: payload.userId });
    await reply('Permission denied: switching models requires admin access.');
    return true;
  }
