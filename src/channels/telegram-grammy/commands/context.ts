 ...
 * fingerprint.
 *
 * Everything here does only DB reads (now async on this tree), so it is safe
 * to call from a menu fingerprint function or a dynamic range factory - the
 * menu plugin accepts MaybePromise there, and requires the results to be pure
 * and stable (no side effects, no fast-changing data).
 *
...
export async function contextMessagingGroupId(ctx: Context): Promise<string | null> {
  const platformId = contextPlatformId(ctx);
  if (!platformId) return null;
  const mg = await getMessagingGroupByPlatform(CHANNEL, platformId);
  return mg ? mg.id : null;
}

/** Resolve the wired agent group(s) for this update's chat/topic. */
export async function resolveTargetsForContext(ctx: Context): Promise<TargetResolution> {
...
export async function contextTargetAgent(ctx: Context): Promise<TargetAgent | null> {
  return agentAtIndex(await resolveTargetsForContext(ctx), readAgentIndex(ctx));
}
 ...
 */
export async function menuFingerprint(ctx: Context): Promise<string> {
  ...
  if (target) {
    ...
    const view = await getConfigView(
      target.agentGroupId,
      mgId ? { messagingGroupId: mgId, threadId: null } : undefined,
    );
    if (view.ok) {
      const v = view.view;
 ...
 */
export async function pickerFingerprint(ctx: Context): Promise<string> {
  return `pick#${wiringIds(await resolveTargetsForContext(ctx)).join('|')}`;
}
