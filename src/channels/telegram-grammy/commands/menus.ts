 ...
 *    and the plugin can match the pressed button to its handler. The plugin
 *    accepts MaybePromise for both `fingerprint` and `dynamic`, so the async
 *    DB layer folds in directly.
...
export function buildMenus(runtime: AdapterRuntime): CommandMenus {
  const options = { autoAnswer: false as const, fingerprint: (ctx: Context): Promise<string> => menuFingerprint(ctx) };
  const pickerOptions = {
    autoAnswer: false as const,
    fingerprint: (ctx: Context): Promise<string> => pickerFingerprint(ctx),
  };
  ...
  const chatCtxOf = async (
    ctx: Context,
  ): Promise<{ messagingGroupId: string; threadId: string | null } | undefined> => {
  ...
  const applyChange = <V>(
    ctx: Context,
    run: (agentGroupId: string, actor: string) => Promise<CommandResult<V>>,
    confirm: (view: V) => string,
    toast: string,
  ): Effect.Effect<void> =>
    Effect.gen(function* () {
      const target = yield* Effect.promise(() => contextTargetAgent(ctx));
      if (!target) return yield* answer(ctx, { text: 'No agent here', show_alert: true });
      const res = yield* Effect.promise(() => run(target.agentGroupId, actorUserId(ctx)));
  ...
  // text refresh so the submenu prompt matches its buttons. Accepts the
  // promise `promptFor` now returns (async DB reads).
  const navRefresh = (ctx: Context, markdown: string | Promise<string>): Effect.Effect<void> =>
    Effect.gen(function* () {
      const text = yield* Effect.promise(() => Promise.resolve(markdown));
      yield* editText(ctx, text);
      yield* answer(ctx);
    });
  ...
  const promptFor = async (
    ...
    const target = await contextTargetAgent(ctx);
    const name = target?.agentName ?? 'agent';
    if (kind === 'model') {
      const picker = target ? await getModelPicker(target.agentGroupId) : null;
    ...
    const view = target ? await getConfigView(target.agentGroupId, await chatCtxOf(ctx)) : null;
  ...
  modelMenu.dynamic(async (ctx, range) => {
  ...
  effortMenu.dynamic(async (ctx, range) => {
    const idx = idxPayload(ctx);
    const target = await contextTargetAgent(ctx);
    const view = target ? await getConfigView(target.agentGroupId) : null;
  ...
  windowMenu.dynamic(async (ctx, range) => {
    const idx = idxPayload(ctx);
    const target = await contextTargetAgent(ctx);
    const view = target ? await getConfigView(target.agentGroupId) : null;
  ...
  activationMenu.dynamic(async (ctx, range) => {
    const idx = idxPayload(ctx);
    const target = await contextTargetAgent(ctx);
    const view = target ? await getConfigView(target.agentGroupId, await chatCtxOf(ctx)) : null;
      ...
      if (mode === 'pattern') {
        ...
        range.text({ text: label, payload: idx }, (c) =>
          runtime.runPromise(
            applyChange(
              c,
              async (ag, actor) => setActivation((await contextMessagingGroupId(c)) ?? '', ag, mode, null, actor),
              activationChangeConfirmation,
              'Activation updated',
            ),
          ),
  ...
  pickModel.dynamic(async (ctx, range) => {
    const res = await resolveTargetsForContext(ctx);
  ...
  pickConfig.dynamic(async (ctx, range) => {
    const res = await resolveTargetsForContext(ctx);
    const agents = res.kind === 'multiple' ? res.agents : [];
    agents.forEach((a, i) => {
