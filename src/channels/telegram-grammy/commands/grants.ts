 ...
 */
export async function computeCommandGrants(): Promise<CommandGrant[]> {
  // Global admin set (owners + global admins) applies to every wired chat.
  const globalAdminUserIds = new Set<string>();
  for (const r of await getOwners()) globalAdminUserIds.add(r.user_id);
  for (const r of await getGlobalAdmins()) globalAdminUserIds.add(r.user_id);
  ...
  const scopedAdminsOf = async (agentGroupId: string): Promise<string[]> => {
    let ids = scopedAdminCache.get(agentGroupId);
    if (!ids) {
      ids = (await getAdminsOfAgentGroup(agentGroupId)).map((r) => r.user_id);
