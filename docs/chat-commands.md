...
unrecognized `!word` is left untouched, since `!` is common in prose. The
pass-through admin commands work the same way (`!compact`, `!clear`, ...), and
`/new` (or `!new`) is an alias for `/clear` on every channel.
...
confirmation (for example "Switched to Opus 5, applies from her next reply").
